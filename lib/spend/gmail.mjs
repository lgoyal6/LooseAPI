...
// Google's endpoints fail to connect occasionally, and one such failure took
// the whole run down at 06:16 on Sep 4 2026 even though the scan itself was
// fine. Retry the transport, never the verdict: an HTTP error is an answer and
// is handled below, only a failure to reach the host at all is worth repeating.
async function fetchRetry(url, init, attempts = 3) {
  for (let n = 1; ; n += 1) {
    try {
      ...
      await new Promise((wait) => setTimeout(wait, n * 1500));
    }
  }
}
 ...
 */
/**
 * Every spend event is billing mail, so it all lands under the single approved
 * `Money` label. An earlier version mapped to Money/{Charges,Credits,Trials} —
 * a taxonomy that mirrored this parser's event classes and was rejected for
 * exactly that reason. The six-bucket inbox scheme lives in lib/mail/rules.mjs.
 */
const MONEY_LABEL = "Money";

async function labelIdMap(auth) {
  const res = await fetchRetry("https://gmail.googleapis.com/gmail/v1/users/me/labels", { headers: auth });
...
async function ensureLabel(auth, name, existing) {
  ...
  const res = await fetchRetry("https://gmail.googleapis.com/gmail/v1/users/me/labels", {
    method: "POST",
...
export async function applyLabels(events) {
  ...
  for (const e of events) {
    const name = MONEY_LABEL;
    if (e.missingFromMailbox) {
      skipped++;
      continue;
    }
    try {
