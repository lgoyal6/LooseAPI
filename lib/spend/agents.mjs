...
export function report(usage, { top = 8, window = 7 } = {}) {
  ...
  return {
    ...
    // Not truncated to `top`. There are four logins and the whole question a
    // per-account row answers is which one is carrying the cost, so a cut-off
    // that hid the smallest would hide exactly the account somebody forgot
    // they were still paying for.
    byAccount: rank(mergeBy(tools, "byAccount"), Infinity),
