...
export function extractEvent(msg, services = []) {
  ...
  return {
    ...
    // null, not false, when the labels are unknown. A message that arrived by
    // forwarding carries no knowledge of what became of the copy left in the
    // inbox, and `(labelIds || [])` would quietly answer "read" to a question
    // nobody can answer. The unread alert tests for true, so unknown skips it
    // instead of being reported as seen.
    unread: msg.labelIds == null ? null : msg.labelIds.includes("UNREAD"),
    trashed: msg.labelIds == null ? null : msg.labelIds.includes("TRASH"),
  };
}
