...
export function extractEvent(msg, services = []) {
  ...
  const inDictionary = services.some((s) => s.domains.some((d) => domainMatches(domain, d)));
  const scope = isPersonal ? "personal" : inDictionary || isProcessor ? "dev" : "unknown";
  ...
  for (const [, list] of byService) {
    ...
    if (credits.length >= 2) {
      ...
      if (days > 0 && spent > 0) {
        ...
        const daysLeft = b.creditsRemainingCents / perDay;
        alerts.push({
          ...
          message:
            ...
            `burning ${fmt(Math.round(perDay))}/day -> ~${daysLeft.toFixed(1)} days`,
          evidence: [a.id, b.id],
        });
      }
    }
