...
export const EVENT_STRONG =
  /\byou.?re invited\b|\bregistration (is )?(confirmed|approved|successful)\b|\brsvp\b|\bis happening (soon|tomorrow|today)\b|\bappointment booked\b|^(invitation|updated invitation|canceled event|reminder):/i;
...
export const RULES = [
  ...
  {
    // Sits above Jobs/Closed so an invitation beats a recruiting domain, and
    // below Jobs/Active so an interview invite is still the deadline it is.
    label: "Events",
